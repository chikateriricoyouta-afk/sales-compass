...
const COMBO_COMMENTS: Partial<Record<ComboKey, string>> = {
  // ※ 文末の対策アドバイスは「最優先で伸ばすポイント」(abilityCombos.ts)と重複するため、
  //    ここでは「どの瞬間に売上を逃しているか」の描写だけに絞っている。
  "empathyConsultant:decisionSupport":
    "お客様との関係を大切にするあまり、最後に踏み込むことで関係が悪くなることを無意識に避けている可能性があります。あなたの場合、強いクロージングを覚える必要はありません。",
  "empathyConsultant:problemFinding":
    "相手の気持ちを尊重できることは強みですが、踏み込んだ質問を遠慮して、表面的なニーズで商談が終わる可能性があります。",
